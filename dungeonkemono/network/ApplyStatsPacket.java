        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (player != null) {
                player.getCapability(PlayerDataProvider.PLAYER_DATA).ifPresent(data -> {
                    // 현재 스탯
                    int currentStr = data.getStats().getStatValue(StatType.STRENGTH);
                    int currentDex = data.getStats().getStatValue(StatType.DEXTERITY);
                    int currentVit = data.getStats().getStatValue(StatType.VITALITY);
                    int currentInt = data.getStats().getStatValue(StatType.INTELLIGENCE);
                    int currentLuk = data.getStats().getStatValue(StatType.LUCK);
                    ...
                    if (usedPoints <= data.getStats().getFreePoints() && usedPoints >= 0) {
                        // 스탯 적용
                        data.getStats().setStatValue(StatType.STRENGTH, newStr);
                        data.getStats().setStatValue(StatType.DEXTERITY, newDex);
                        data.getStats().setStatValue(StatType.VITALITY, newVit);
                        data.getStats().setStatValue(StatType.INTELLIGENCE, newInt);
                        data.getStats().setStatValue(StatType.LUCK, newLuk);
