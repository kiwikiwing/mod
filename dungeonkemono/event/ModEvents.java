...
public class ModEvents {
    ...
    public static void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        if (event.getEntity() instanceof ServerPlayer serverPlayer) {
            serverPlayer.getCapability(PlayerDataProvider.PLAYER_DATA).ifPresent(data -> {
                ...
                // 클라이언트로 데이터 전송
                syncPlayerData(serverPlayer, data);
            });
        }
    }
        ...
        if (event.getSource().getEntity() instanceof ServerPlayer player) {
            player.getCapability(PlayerDataProvider.PLAYER_DATA).ifPresent(data -> {
                ...
                if (exp > 0) {
                    ...
                    // 경험치 추가 & 레벨업 체크
                    int levelUps = data.addExperience(exp);

                    if (levelUps > 0) {
                        sendLevelUpMessage(player, data, levelUps);
                    }
    ...
    /**
     * 플레이어 데이터를 클라이언트로 동기화
     */
    private static void syncPlayerData(ServerPlayer player, PlayerData data) {
        ...
        DungeonKemono.LOGGER.debug("Synced player data to client for: {}", player.getName().getString());
    }
}